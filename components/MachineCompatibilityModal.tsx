...
import { Search, X, Printer, Star } from 'lucide-react';
...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSelectMachine?: (machine: DTFMachineItem) => void;
}
...
export default function MachineCompatibilityModal({ isOpen, onClose, onSelectMachine }: Props) {
  ...
  const filteredMachines = DTF_MACHINES.filter(
    (m: DTFMachineItem) =>
      ...
      <div className="bg-[#161616] border border-[#2E2E2E] rounded-3xl max-w-4xl w-full max-h-[85vh] flex flex-col overflow-hidden shadow-2xl relative">
        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredMachines.map((m: DTFMachineItem) => (
              <div
                key={m.id}
                onClick={() => {
                  if (onSelectMachine) {
              ...
              >
                <div className="space-y-1">
                  ...
                  <div className="text-[11px] text-slate-400 font-mono flex items-center gap-3 pt-1">
                    <span>📐 Résolution : <strong className="text-slate-200">{m.maxResolution}</strong></span>
                  </div>
                </div>

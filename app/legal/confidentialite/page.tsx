      ...
      <main className="max-w-4xl mx-auto px-4 py-16 space-y-8">
        <div className="border-b border-[#2E2E2E] pb-6">
          <h1 className="text-3xl md:text-4xl font-extrabold text-white mb-2">
            {dict.legal?.privacy?.title || dict.legal?.privacyTitle}
          </h1>
          <p className="text-xs text-slate-400">
            {dict.legal?.privacy?.lastUpdate || dict.legal?.lastUpdate} : 04/09/2026
          </p>
        </div>

        <section className="space-y-6 text-xs leading-relaxed text-slate-300">
          {dict.legal?.privacy?.sections?.donnees && (
            <div>
              <h2 className="text-base font-bold text-white mb-2 text-[#F7941D]">
                {dict.legal.privacy.sections.donnees.title}
              </h2>
              <p>{dict.legal.privacy.sections.donnees.content}</p>
            </div>
          )}

          {dict.legal?.privacy?.sections?.traitement && (
            <div>
              <h2 className="text-base font-bold text-white mb-2 text-[#F7941D]">
                {dict.legal.privacy.sections.traitement.title}
              </h2>
              <p>{dict.legal.privacy.sections.traitement.content}</p>
            </div>
          )}

          {dict.legal?.privacy?.sections?.droits && (
            <div>
              <h2 className="text-base font-bold text-white mb-2 text-[#F7941D]">
                {dict.legal.privacy.sections.droits.title}
              </h2>
              <p>{dict.legal.privacy.sections.droits.content}</p>
            </div>
          )}
        </section>
      </main>
